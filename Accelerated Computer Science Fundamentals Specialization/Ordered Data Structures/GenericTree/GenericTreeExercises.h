...
static void treeFactory(GenericTree<int>& tree) {
  ...
  root->data = 4;
  auto left = root->addChild(8);
  auto right = root->addChild(15);
  auto l = left->addChild(16);
  left->addChild(23);
  l->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while(!q.empty())
  {
    ...
    {
      TreeNode * tmpNode = q.front(); // pop out this level treeNodes
      q.pop();
      int vectorSize = tmpNode->childrenPtrs.size();
      for(int j=0 ; j< vectorSize ; ++j)
      {
        results.push_back(tmpNode->childrenPtrs[j]->data); // put this node's child to result stack
        q.push(tmpNode->childrenPtrs[j]); 
      }
    }
  }
